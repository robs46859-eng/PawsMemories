...
| 4. Rig, facial, accessories | Foundation corrected; implementation blocked | Schema 23 key/FK integrity, accepted Phase 3 lineage checks, deterministic classification, measured-only validators, fail-closed worker state, hash-bound review UI | A real worker adapter, canonical rigged output registration/lineage, retries/recovery, measured facial/accessory artifacts, fixture corpus, MySQL/live Blender evidence, and browser matrix remain. Keep `RIG_PIPELINE_V4_ENABLED=false`; evidence: `phase-evidence/PHASE_4.md` |
| 5. Fur Bin showcase | Server foundation corrected; product integration incomplete | Schema 24 canonical owner/version registration, exact version storage totals, owner-scoped search/collections/rollback, admin-gated moderation, approved public-derivative reads | Responsive V5 UI, legacy adapter, canonical measured badges, public derivative creation, marketplace immutable-deliverable linkage, MySQL/storage race tests, and browser matrix remain. Keep `FUR_BIN_V5_ENABLED=false`; evidence: `phase-evidence/PHASE_5.md` |
...
| 9. Scaled shell and IFC | Calibration/verification foundation complete; release integration pending | Default-off calibrated text/image proposal lane; observed-view byte validation; before/after dimension reports; shell-vs-IFC claims/pricing; IFC unique GlobalIds, storeys, property sets, host/fill relationships, CRS label, placement, unit, and reopen gates; 14 new focused tests plus existing BIM/pricing regressions pass | Move legacy BIM artifacts to private signed storage and durable idempotent billing/refund jobs after Phase 4-5 integration; then credentialed Gemini, Render IfcOpenShell, browser/mobile, and release archive evidence |
