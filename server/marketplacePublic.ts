...
export async function checkoutDigital(pool: Pool, userPhone: string, listingUuid: string, idempotencyKey: string) {
  ...
  const [lRows] = await pool.query(
    `SELECT id, digital_price_cents FROM marketplace_listings WHERE uuid = ? AND status = 'published' LIMIT 1`,
    [listingUuid]
  ) as any;
  ...
  return { orderId, priceCents: listing.digital_price_cents, listingId: listing.id, assetId };
}
