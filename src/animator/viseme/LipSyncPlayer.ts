...
import { FacialRigMap } from "../types.ts";
...
export interface LipSyncPlayerOptions {
  ...
  onEnd?: () => void;
  /** The facial rig map generated by the server during validation */
  facialRigMap?: FacialRigMap;
}
...
export class LipSyncPlayer {
  ...
  private opts: Required<Pick<LipSyncPlayerOptions, "getClock" | "crossfadeSec" | "maxJawRad" | "maxCornerRad">> &
    Pick<LipSyncPlayerOptions, "tierA" | "onEnd" | "facialRigMap">;
  ...
  constructor(root: THREE.Object3D, track: VisemeTrack | null, options: LipSyncPlayerOptions = {}) {
    ...
    this.opts = {
      ...
      tierA: options.tierA,
      onEnd: options.onEnd,
      facialRigMap: options.facialRigMap,
    };
  ...
  private collectBindings(): void {
    const map = this.opts.facialRigMap;
    
    this.root.traverse((obj: any) => {
      if (obj.isMesh && obj.morphTargetDictionary && obj.morphTargetInfluences) {
        for (const shape of VISEME_SHAPES) {
          // If map provided, use exact pre-mapped targets
          if (map && map.visemes[shape]) {
            for (const mapping of map.visemes[shape]) {
              const idx = obj.morphTargetDictionary[mapping.morphTarget];
              if (idx !== undefined) {
                const arr = this.morphBindings.get(shape) ?? [];
                arr.push({ mesh: obj as THREE.Mesh, index: idx, multiplier: mapping.weight });
                this.morphBindings.set(shape, arr);
              }
            }
          } else {
            // Legacy fallback if no map is provided
            const name = `viseme_${shape}`;
            const idx = obj.morphTargetDictionary[name];
            if (idx !== undefined) {
              const arr = this.morphBindings.get(shape) ?? [];
              arr.push({ mesh: obj as THREE.Mesh, index: idx, multiplier: 1.0 });
              this.morphBindings.set(shape, arr);
            }
          }
        }
    ...
    if (map) {
      this.jawBone = map.jawBone ? this.root.getObjectByName(map.jawBone) ?? null : null;
      this.cornerLBone = map.lipCornerLeftBone ? this.root.getObjectByName(map.lipCornerLeftBone) ?? null : null;
      this.cornerRBone = map.lipCornerRightBone ? this.root.getObjectByName(map.lipCornerRightBone) ?? null : null;
    } else {
      this.jawBone = this.root.getObjectByName("jaw") ?? null;
      this.cornerLBone = this.root.getObjectByName("lipCorner.L") ?? null;
      this.cornerRBone = this.root.getObjectByName("lipCorner.R") ?? null;
    }
  ...
  private writeFace(): void {
    ...
    for (const shape of VISEME_SHAPES) {
      ...
      for (const b of bindings) {
        (b.mesh.morphTargetInfluences as number[])[b.index] = w * b.multiplier;
      }
    }
