...
export default function BimModelBuilder({ onClose, userProfile, onUpdateUser }: { onClose: () => void; userProfile: UserProfile; onUpdateUser: (user: UserProfile) => void }) {
  ...
  const handleBuild = async () => {
    ...
    try {
      ...
      if (BIM_V2_ENABLED) {
        const expectedClaim = buildMode === "ifc" ? "verified_ifc4_semantic_model" : "verified_scaled_visual_shell";
        if (!result.postBuild?.passed || result.postBuild.claim !== expectedClaim) throw new Error("The delivered model did not earn the requested verification claim.");
        if (result.postBuild.preBuildReportHash !== preflight.reportHash) throw new Error("The delivered model is not bound to the pre-build report you reviewed.");
        if (result.postBuild.modelHash !== preflight.modelHash || result.postBuild.calibrationHash !== preflight.calibrationHash) throw new Error("The delivered model is not bound to the reviewed model and calibration.");
        if (!/^[a-f0-9]{64}$/i.test(result.postBuild.outputSha256 || "")) throw new Error("The delivered model is missing its output integrity hash.");
      }
      ...
      setMessage(`${buildMode === "ifc" ? "IFC/BIM" : "Shell"} model passed post-build verification.${userProfile.isAdmin ? " No credits were charged to the administrator account." : ` ${price} credits were charged.`}`);
    } catch (error: any) { setMessage(`${error.message} Check the build record for the final credit or refund state.`); } finally { setBusy(""); }
  };
  ...
  return <div className="fixed inset-0 z-[90] bg-[#efe9dc] text-[#28302c] overflow-auto">
    ...
    <main className="grid min-h-[calc(100vh-65px)] lg:grid-cols-[280px_1fr_320px]">
      ...
      <aside className="border-l border-[#28302c]/15 p-5 sm:p-6 lg:p-4">
        ...
        <div className="mb-3 rounded-xl border bg-white p-3 text-xs"><strong>Gate 1 · Before build</strong><p className={preflight?.passed ? "text-green-700" : "text-[#59655f]"}>{preflight ? preflight.passed ? "Passed" : "Failed" : "Not run"}</p>{preflight?.warnings?.map((warning: string) => <p key={warning} className="mt-1 text-amber-700">{warning}</p>)}{preflight?.dimensionComparisons?.map((item: any) => <p key={item.id} className={item.passed ? "mt-1 text-green-700" : "mt-1 text-red-700"}>{item.axis}: {item.actualMeters?.toFixed(3)} m / {item.expectedMeters.toFixed(3)} m{item.uncertaintyMeters == null ? " · uncertainty not supplied" : ` ± ${item.uncertaintyMeters.toFixed(3)} m`}</p>)}{preflight?.reportHash && <p className="mt-2 break-all text-[9px] text-[#59655f]">Review hash: {preflight.reportHash}</p>}<strong className="mt-2 block">Gate 2 · After build</strong><p className={postBuild?.passed ? "text-green-700" : "text-[#59655f]"}>{postBuild ? postBuild.passed ? `Passed · ${postBuild.format}` : "Failed; check the build record for credit status" : "Runs after construction"}</p>{postBuild?.dimensionComparisons?.map((item: any) => <p key={item.id} className={item.passed ? "mt-1 text-green-700" : "mt-1 text-red-700"}>{item.axis}: Δ {item.deltaMeters?.toFixed(3)} m (limit {item.toleranceMeters.toFixed(3)} m)</p>)}{postBuild?.outputSha256 && <p className="mt-2 break-all text-[9px] text-[#59655f]">Output hash: {postBuild.outputSha256}</p>}</div>
